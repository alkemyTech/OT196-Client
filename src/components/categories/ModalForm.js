...
const categorieSchema = Yup.object().shape({
  ...
  description: Yup.string().max(
    75,
    "El nombre no puede tener más de 75 caracteres"
  ),
});
...
function ModalForm({ categoryData, lastUpdate, handleClose, setIsLoading }) {
  ...
  const initialValues =
    formType === "Edit" ? categoryData : { name: "", description: "" };
        ...
        try {
          const reqPath =
            formType === "Create"
              ...
              : `${REACT_APP_BACKEND_CATEGORIES}/${categoryData.id}`;
          await saveFunction[formType](reqPath, { name, description });
          successAlert({
            iconSuccess: "success",
            titleSuccess: "¡Todo correcto!",
            msgSuccess: "Se han guardado los cambios con exito.",
          });
          setIsLoading(false);
          handleClose();
          lastUpdate(Date.now());
          ...
          errorAlert({
            ...
            msgError: "No ha sido posible realizar los cambios.",
          });
          setIsLoading(false);
        }
      }}
      ...
      {({ handleSubmit }) => (
        <Container className="mt-2">
          <FormikForm noValidate id="categoryModal" onSubmit={handleSubmit}>
            <InputEdit label="Nombre" name="name" />
            <InputEdit label="Descripción" name="description" />
          </FormikForm>
        </Container>
      )}
    </Formik>
